...
def plot_skeleton(V, E, plot_point=False):
    ...
    if not plot_point:
        open3d.visualization.draw_geometries([line_set], "Skeleton")
    else:
...
def save_graph(filename, V, E, n, leaf_width, matlab_type=False):
    # Concatenate V (vertices), n (normals), and leaf_width into a single array
    vn = np.hstack((V, n, leaf_width.reshape(-1, 1)))

    with open(filename, "w") as file:
        # Save each vertex with its normal and leaf_width
        for i in range(V.shape[0]):
            # Write the vertex data: position, normal, and leaf_width
            file.write("v " + " ".join([str(x) for x in list(vn[i])]) + "\n")
