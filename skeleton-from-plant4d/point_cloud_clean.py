...
def clean_point_cloud(pcd, option={}, verbose=False, translate=True):
    ...
    description = get_description_point_cloud(pcd,False)
    raw_coord_mean = description['raw_coord_mean']
    if translate:
        pcd.translate(-description['raw_coord_mean'])
    ...
    description = get_description_point_cloud(pcd_down, True)
    description['voxel_size'] = voxel_size  # collect the description of point cloud cleaned
    open3d.io.write_point_cloud("D:/4d_plant_analysis_data/data/maize/processed/with_normals/ds_pcd.ply", pcd_down, write_ascii=True)
    #pcd_np = np.asarray(pcd.points)
    pcd.translate(-description['coord_mean'])

    return pcd_down, description, raw_coord_mean


def get_description_point_cloud(pcd, ds = True):
    """
    ...
    if ds:
        ...
        description['raw_coord_mean'] = np.mean(points, axis=0)
    return description
