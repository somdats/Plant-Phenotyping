...
def register_skeleton(S1, S2, corres, params):
  ...
  for i in range(params['num_iter']):
    ...
    for j in range(m):
      ...
      adj_tot = np.sum(S1.A[j,:])
      for k in range(adj_tot):
      ...
      if corres.shape[0] > 0:
        ...
        if len(ind_C) > 0:
          ...
          #incoporating new changes
          normal = S1.normals[j,:].reshape(3,1)
          #J_jc_corresp, r_jc_corresp = compute_corresp_constraints_new(Rj, tj, Y, normal, w_corres=100, w_p2pl=1.0)
          ...
          #print( "j,k:",j,k)
          # print("nc, j_corresp, jc, j_jc_correp", nc*jc, nc*(jc+1), 12*j, 12*(j+1))
          # print("shape of J_corres:", J_corresp.shape)
          # print("J_jc_corresp shape:", J_jc_corresp.shape)
          J_corresp[nc*jc:nc*(jc+1), 12*j:12*(j+1)] = J_jc_corresp
          #J_corresp[nc*jc:nc*(jc+1), 12*j:12*(j+1)] = J_jc_corresp
          # start_row = nc * jc
          # end_row = start_row + J_jc_corresp.shape[0]  # Adjust the end row to match the shape of J_jc_corresp
          # J_corresp[start_row:end_row, 12*j:12*(j+1)] = J_jc_corresp
          r_corresp[nc*jc:nc*(jc+1)] = r_jc_corresp

          #plot residual
          #if( i == params['num_iter'] - 1):
          #plot_residual(r_corresp,j)
...
def plot_residual(r_corresp,k):
    # Analyze the magnitude of residuals
    residual_magnitudes = np.linalg.norm(r_corresp, axis=1)  # assuming r_corresp is N x 3 for N correspondences

    # Plot the distribution of residuals
    plt.figure()
    plt.hist(residual_magnitudes, bins=50)
    plt.title('Distribution of Residual Magnitudes')
    plt.xlabel('Residual Magnitude')
    plt.ylabel('Frequency')
    plt.show()

    # Standardized residuals (assuming residuals are normally distributed)
    residual_mean = np.mean(residual_magnitudes)
    residual_std = np.std(residual_magnitudes)
    standardized_residuals = (residual_magnitudes - residual_mean) / residual_std

    # Identify outliers
    outliers = np.abs(standardized_residuals) > 2  # or 3 for a stricter threshold

    # Goodness-of-Fit
    rmse = np.sqrt(np.mean(residual_magnitudes**2))
    mae = np.mean(np.abs(residual_magnitudes))

    print(f"RMSE: {rmse}")
    print(f"MAE: {mae}")
    print(f"inner_iteration: {k}")

def compute_corresp_constraints_new(R, t, Y, normal, w_corres=100, w_p2pl=1.0):
    # Ensure normal is a column vector
    normal = normal.reshape((3, 1)) if normal.ndim == 1 else normal

    # Compute point-to-point residuals and Jacobian
    r_pt2pt = R @ Y.g + t - Y.q  # Point-to-point residual
    r_pt2pt *= w_corres  # Apply weighting

    # Construct the Jacobian for point-to-point
    JR = np.hstack((Y.g[0] * np.eye(3), Y.g[1] * np.eye(3), Y.g[2] * np.eye(3))) * w_corres
    Jt = np.eye(3) * w_corres
    J_pt2pt = np.hstack((JR, Jt))

    # Calculate point-to-plane error for each point
    pt2pl_error = np.dot(normal.T, (R @ Y.g + t - Y.q)) * w_p2pl
    r_pt2pl = pt2pl_error.reshape(-1, 1)  # Reshape to column vector

    # Construct the Jacobian for point-to-plane
    J_pt2pl = np.hstack((normal.T @ JR, normal.T)) * w_p2pl
    J_pt2pl = J_pt2pl.reshape(1, -1)  # Ensure it's a 2D array (1x12)

    # Combine the residuals and Jacobians
    r_combined = np.vstack((r_pt2pt, r_pt2pl))
    J_combined = np.vstack((J_pt2pt, J_pt2pl))

    return J_combined, r_combined
...
def apply_registration_params_to_skeleton(S, T):
  ...
  m = S.XYZ.shape[0]
  for j in range(m):
    S_hat.XYZ[j,:3] = hf.hom2euc(T[j] @ hf.euc2hom(np.reshape(S.XYZ[j,:3], (3,1)))).T
 
  return S_hat

def compute_skeleton_registration_error(S1, S2, corres, T12):
  S1_hat = apply_registration_params_to_skeleton(S1, T12)
  err = np.linalg.norm(S1_hat.XYZ[corres[:,0] ,:] - S2.XYZ[corres[:,1] ,:], axis = 1)
  return err  
